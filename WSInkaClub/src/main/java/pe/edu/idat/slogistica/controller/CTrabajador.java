...
import pe.edu.idat.slogistica.service.STrabajador;

@RestController
@RequestMapping("/slogistica/trabajadores")
public class CTrabajador {
	@Autowired
	private STrabajador controller;
	...
	public ResponseEntity<Object> buscar(@PathVariable(name="id")int id){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Trabajador no encontrada");
		return new ResponseEntity<Object>(controller.buscar(id),HttpStatus.OK);
	}
	
	@PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Object> crear(@RequestBody MTrabajador trabajador){
		String respuesta = controller.registrar(trabajador);
		if(respuesta==null)
	...
	public ResponseEntity<Object> actualizar(@PathVariable(name="id") int id, @RequestBody MTrabajador trabajador){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Trabajador no encontrada");
		String respuesta = controller.actualizar(trabajador);
		if(respuesta == null)
