...
import pe.edu.idat.slogistica.service.SCargo;

@RestController
@RequestMapping("/slogistica/cargos")
public class CCargo {
	
	@Autowired
	private SCargo controller;
	...
	public ResponseEntity<Object> buscar(@PathVariable(name="id")int id){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Cargo no encontrada");
		return new ResponseEntity<Object>(controller.buscar(id),HttpStatus.OK);
	}
	
	@PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Object> crear(@RequestBody MCargo cargo){
		String respuesta = controller.registrar(cargo);
		if(respuesta==null)
	...
	public ResponseEntity<Object> actualizar(@PathVariable(name="id") int id, @RequestBody MCargo cargo){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Cargo no encontrada");
		String respuesta = controller.actualizar(cargo);
		if(respuesta == null)
