...
import pe.edu.idat.slogistica.service.STipoCarga;

@RestController
@RequestMapping("/slogistica/tipocargas")
public class CTipoCarga {
	
	@Autowired
	private STipoCarga controller;
	...
	public ResponseEntity<Object> buscar(@PathVariable(name="id")int id){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"TipoCarga no encontrada");
		return new ResponseEntity<Object>(controller.buscar(id),HttpStatus.OK);
	}
	
	@PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Object> crear(@RequestBody MTipoCarga tipocarga){
		String respuesta = controller.registrar(tipocarga);
		if(respuesta==null)
	...
	public ResponseEntity<Object> actualizar(@PathVariable(name="id") int id, @RequestBody MTipoCarga tipocarga){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"TipoCarga no encontrada");
		String respuesta = controller.actualizar(tipocarga);
		if(respuesta == null)
