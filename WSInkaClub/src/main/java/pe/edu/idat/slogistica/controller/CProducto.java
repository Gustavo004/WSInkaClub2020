...
import pe.edu.idat.slogistica.service.SProducto;

@RestController
@RequestMapping("/slogistica/productos")
public class CProducto {
	
	@Autowired
	private SProducto controller;
	...
	public ResponseEntity<Object> buscar(@PathVariable(name="id")int id){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Producto no encontrada");
		return new ResponseEntity<Object>(controller.buscar(id),HttpStatus.OK);
	}
	
	@PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Object> crear(@RequestBody MProducto producto){
		String respuesta = controller.registrar(producto);
		if(respuesta==null)
	...
	public ResponseEntity<Object> actualizar(@PathVariable(name="id") int id, @RequestBody MProducto producto){		
		if(controller.buscar(id) == null)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND,"Producto no encontrada");
		String respuesta = controller.actualizar(producto);
		if(respuesta == null)
